...
| `maxai` | `mx` | MaxAI | Web cookie | [link](https://www.maxai.co) | Sign in with an email code or import credentials manually. OmniRoute signs each request, uses the required Windows Firefox 150 client profile, follows your configured connection/proxy, and refreshes the daily access token from the roughly yearly refresh token. | emulated |
